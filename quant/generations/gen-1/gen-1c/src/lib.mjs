// Gen-1C Core Trading Library (Environment Mutation — 15-min candles)
import fs from 'fs';
...
// === Learning Loop (expanded triggers + proposal mechanism) ===
export function checkLearningLoop(state) {
  const now = Date.now();
  const COOLDOWN = 2 * 60 * 60 * 1000; // 2h cooldown between learning loops
  ...
  if (triggered) {
    ...
    fs.appendFileSync(LEARNING_LOG, learningEntry);
...
function writeProposal(state, triggerReason) {
  ...
  if (triggerReason.includes('consecutive losses')) {
    type = 'cognition';
    summary = 'Consider tighter entry conditions or additional confirmation signals';
    evidence = `${state.consecutiveLosses} consecutive losing trades detected`;
    impact = 'Fewer but higher-quality trades, reduced loss streaks';
    risk = 'May miss profitable opportunities by being too selective';
    ...
    type = 'environment';
    summary = 'Market may be in low-volatility regime requiring wider entry conditions';
    evidence = triggerReason;
    impact = 'More trade opportunities in quiet markets';
    risk = 'Wider conditions may reduce signal quality';
  }

  const proposal = `\n## Proposal [${now}]\n### Type: ${type}\n### Summary: ${summary}\n### Evidence: ${evidence}\n### Expected Impact: ${impact}\n### Risk: ${risk}\n\n`;

  if (!fs.existsSync(PROPOSALS_LOG)) {
    fs.writeFileSync(PROPOSALS_LOG, '# Gen-1C Proposals\n');
  }
...
export function initTradesLog() {
  ...
  if (!fs.existsSync(TRADES_LOG)) {
    fs.writeFileSync(TRADES_LOG, '# Gen-1C Trades Log\n| Time | Action | Price | Qty | PnL | Reason | Cash |\n|------|--------|-------|-----|-----|--------|------|\n');
  }
  if (!fs.existsSync(LEARNING_LOG)) {
    fs.writeFileSync(LEARNING_LOG, '# Gen-1C Learning Log\n');
  }
  if (!fs.existsSync(PROPOSALS_LOG)) {
    fs.writeFileSync(PROPOSALS_LOG, '# Gen-1C Proposals\n');
  }
