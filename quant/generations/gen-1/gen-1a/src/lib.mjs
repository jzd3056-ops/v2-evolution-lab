// Gen-1A Core Trading Library (Control — identical strategy to Gen-0)
import fs from 'fs';
...
function writeProposal(state, triggerReason) {
  ...
  if (!fs.existsSync(PROPOSALS_LOG)) {
    fs.writeFileSync(PROPOSALS_LOG, '# Gen-1A Proposals\n');
  }
...
export function initTradesLog() {
  ...
  if (!fs.existsSync(TRADES_LOG)) {
    fs.writeFileSync(TRADES_LOG, '# Gen-1A Trades Log\n| Time | Action | Price | Qty | PnL | Reason | Cash |\n|------|--------|-------|-----|-----|--------|------|\n');
  }
  if (!fs.existsSync(LEARNING_LOG)) {
    fs.writeFileSync(LEARNING_LOG, '# Gen-1A Learning Log\n');
  }
  if (!fs.existsSync(PROPOSALS_LOG)) {
    fs.writeFileSync(PROPOSALS_LOG, '# Gen-1A Proposals\n');
  }
